...
namespace WebPatter.WebUI.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private PatternContext dbContext = new PatternContext();
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            User user = dbContext.Users.FirstOrDefault(u => u.Email == model.Email && u.Password == model.Password);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            dbUser.ActivationDate = DateTime.Now;
            dbUser.AccountId = 1;
            var anyUser = dbContext.Users.Any(p => string.Compare(p.Email, dbUser.Email) == 0);
            if (anyUser)
            {
                ModelState.AddModelError("Email", "Пользователь с такой почтой  уже существует");
            }
            var anyUserPhone = dbContext.Users.Any(p => string.Compare(p.Phone, dbUser.Phone) == 0);
            if (anyUserPhone)
            {
